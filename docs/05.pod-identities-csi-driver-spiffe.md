...
Several environment variables are used to setup the SPIFFE driver that is configured in `settings.sh`. One specific configuration that is not obvious from the chart is the `secretName` mounted as `root-cas`. 
For more information about this can be found [here](https://cert-manager.io/docs/projects/csi-driver-spiffe/#root-ca-bundle). 

> Specifically before installing make sure that all the root CA's required are in PEM form and set in `JS_VENAFI_INTERMEDIATE_CA_ROOT_PEM_FILE`. 
In this demo the PEM file holds the root certificate of the private PKI. A secret called `spiffe-source-ca` will be created before install. The CSI SPIFFE driver will create the `ca.crt` based on what's in this CA bundle in every pod. 
