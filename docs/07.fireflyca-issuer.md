...
Run the following , 
```
kubectl -n sandbox get cr -o json | jq -r '.items[] | select(.metadata.name | test("cert-ten-")).status.certificate' |  base64 --decode | openssl x509 -text | grep  CN=cert-ten-days-1.svc.cluster.local -B4
```
to see
...
```
        ...
        Validity
            Not Before: May 21 01:17:10 2023 GMT
            Not After : May 31 01:17:10 2023 GMT
        Subject: C=USA, ST=TX, L=Frisco, O=Venafi Inc, OU=Firefly Unit 2, CN=cert-ten-days-1.svc.cluster.local
```

Run the following, 
```
kubectl -n sandbox get cr -o json | jq -r '.items[] | select(.metadata.name | test("cert-two-")).status.certificate' |  base64 --decode | openssl x509 -text | grep  CN=cert-two-days-1.svc.cluster.local -B4
```
...
```
        Issuer: C=US, ST=TX, L=Frisco, O=Venafi Inc, OU=Firefly Unit, CN=2023-05-20-2014 firefly-built-in-180.svc.cluster.local
        Validity
            Not Before: May 21 01:17:09 2023 GMT
            Not After : May 23 01:17:09 2023 GMT
        Subject: C=USA, ST=TX, L=Frisco, O=Venafi Inc, OU=Firefly Unit 3, CN=cert-two-days-1.svc.cluster.local
```
Run the following
```
kubectl -n sandbox get cr -o json | jq -r '.items[] | select(.metadata.name | test("cert-hundred-")).status.certificate' |  base64 --decode | openssl x509 -text | grep  CN=cert-hundred-days-1.svc.cluster.local -B4
```
to see
```
        Issuer: C=US, ST=TX, L=Frisco, O=Venafi Inc, OU=Firefly Unit, CN=2023-05-20-2014 firefly-built-in-180.svc.cluster.local
        Validity
            Not Before: May 21 01:17:10 2023 GMT
            Not After : Aug 29 01:17:10 2023 GMT
