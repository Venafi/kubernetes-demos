...
echo "[create-sample-data] Creating namespace 'sandbox' for certs and workloads"
...
for i in $(seq 1 ${NUM_SAMPLE_CERTS_TO_CREATE}); do
  create_client_certificate "${i}-${RESOURCE_SUFFIX}.svc.cluster.local"
done

for i in $(seq 1 ${NUM_SAMPLE_CERTS_TO_CREATE}); do
  k=$(openssl rand -hex 8)
  v=$(openssl rand -hex 16)
  kubectl create secret generic ${i}-new-secret \
    --namespace="${KUBERNETES_NAMESPACE}" \
    --from-literal=key="${k}" \
    --from-literal=value="${v}" \
...
echo ""
echo "🚀 What's next:"
echo "→ Access the Secrets Manager (https://<your-tenant>.venafi.cloud)"
echo "   Navigate to: *Discovery and Context Service*"
